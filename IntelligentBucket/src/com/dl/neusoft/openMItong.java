 ...
 */
@WebServlet("/openMitong")
public class openMItong extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(list != null && list.size()>0) {
			...
			String updateSql = "";
			if("1".equals(status)) {
				updateSql = "update mitong set status = 0 where id = 1";
			}else {
				updateSql = "update mitong set status = 1 where id = 1";
			}
